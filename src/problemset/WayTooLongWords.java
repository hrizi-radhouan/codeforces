 ...
 * 
 * Output Print n lines. The i-th line should contain the result of replacing of
...
public class WayTooLongWords {
	...
	public static void main(String[] args) {
		...
		StringBuilder[] words = new StringBuilder[in.nextInt()];
		in.nextLine();
		for (int i = 0; i < words.length; i++) {
			StringBuilder word = new StringBuilder(in.nextLine());
			boolean isLong = word.length() > 10;
			if (isLong) {
				// abbreviation
				word = new StringBuilder(word.charAt(0)+(word.length() - 2)+word.charAt(word.length() - 1));
